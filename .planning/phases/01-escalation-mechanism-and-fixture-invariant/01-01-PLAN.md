...
base moved forward past a real change satisfies all four: that is precisely why
`030521b` passes them while `fcebad1` does not, the difference between the two
being `868d45b`, which modified `.github/workflows/ci.yml`. A criterion whose
left edge is adjustable can always be satisfied by adjusting it.
...
it — the cycle-1 substitution was recorded in detail and approved by no one.
