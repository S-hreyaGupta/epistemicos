...
| `git diff --name-only` with no revision range is vacuous | LOW | Anchored to the phase base commit `868d45b`, which is recorded in **Verified ground truth** and asserted by Task 1's `<precondition>`. |
...
| Phase base `fcebad1` is mis-anchored | HIGH | Re-anchored to `868d45b`, approved by Alex Zamurko on 2026-09-01. Cycle 1 re-anchored to `030521b` instead; that value was recorded but never approved, and it is a commit the review process itself wrote 23 seconds after the review that demanded the move, so anchoring the phase to it would be circular. `868d45b` is the last commit on this branch to touch either gate file, so the ci.yml change sits *at* the anchor rather than behind it. Owned by 01-03, but this plan carries the guard: Task 1's `<precondition>` asserts the base is valid **before any Phase 1 code lands**, so a wrong base fails at phase start rather than at audit time. |
...
commands. Every measurement below holds unchanged at the approved phase base
`868d45b`: `git diff --name-only 868d45b 030521b` lists nothing outside
`.planning/`, and `git rev-parse 868d45b:internal` equals `git rev-parse
030521b:internal`, so the two commits present an identical source tree. Do not
re-derive; do not assume anything beyond these.
...
- **Phase base commit: `868d45bf873c9e147f18a099171b99afa348f4a3`.** Verified at
  ...
  nothing, and `git diff --name-only 868d45b..HEAD` filtered against `^internal/`
  and `^\.planning/` prints nothing. Approved by Alex Zamurko on 2026-09-01. The
  cycle-1 base `fcebad1` failed both of the last two checks, because this base —
  `868d45b` — landed after it and modified `.github/workflows/ci.yml`. Anchoring
  here puts that change *at* the anchor: `868d45b` is the last commit on this
  branch to touch either gate file, which makes it the tightest available anchor
  rather than one reached by stepping past a change. Ancestry alone is not a
  sufficient guard, which is why Task 1 carries a `<precondition>` asserting all
  four properties, and a fifth requiring the base to be an approved value.
- `go test ./... -count=1` at the phase base exits 0 with **38 test-level skips**
  ...
  — 38 total, re-measured at `868d45b`.
...
prints nothing; and `git diff --name-only 868d45bf873c9e147f18a099171b99afa348f4a3..HEAD`
...
report. Do not repair it by moving the anchor: a replacement base is a human
decision, recorded in `phase_base_approval` and re-frozen, never a value the
executor picks — least of all the current branch tip, which is whatever the
process last wrote and is therefore always available to make the check pass.

**The four properties are necessary, not sufficient — assert a fifth.** They
catch a *wrong anchor*. They do not catch an *ungated substitution*, because a
base moved forward past a real change satisfies all four: `fcebad1` fails them
and every commit from `868d45b` onward passes them, the difference being
`868d45b` itself, which modified `.github/workflows/ci.yml`. Cycle 1 took that
route — it moved the anchor to `030521b`, a commit the review process wrote 23
seconds after the review that demanded the move, and the check went green. A
criterion whose left edge is adjustable can always be satisfied by adjusting it.

So assert, fifth and last: the base in use is byte-equal to
`phase_base_approval.commit` in `01-PLAN-MANIFEST.json`, **and**
`phase_base_approval.status` is exactly `approved`. Halt on either failure. A
status of `awaiting_approval` is a halt, not a warning: it means the anchor this
audit rests on was substituted without anyone signing for it, and the audit
cannot be trusted until someone does. Recording a substitution is not approving
it — the cycle-1 substitution to `030521b` was recorded in detail and approved by
no one, and was ultimately rejected in favour of `868d45b`.

Substituting the base is a `checkpoint:decision`, never an executor judgement
call. If the base must change, halt, obtain an explicit human approval naming
the new value and the reason it is not the merge-base, write that into
`phase_base_approval`, re-freeze, and only then resume.
  </precondition>
...
let `go vet ./...` name them — `go build ./...` does not compile `_test.go` files
and exits 0 with an unused test import still present, so it will name nothing here.
This routes all 33 store call sites through the
...
the mechanism makes those tests fail — untestable.
  ...
  <acceptance_criteria>
    ...
    - `git diff --name-only 868d45bf873c9e147f18a099171b99afa348f4a3 -- | grep -v '^internal/' | grep -v '^\.planning/'` prints nothing — the diff is anchored to the phase base, not to an empty range.
  </acceptance_criteria>
...
the rest of each file, so do not guess: run `go vet ./...`, let it name each
unused import, and remove exactly those. Do not use `go build ./...` for this —
it does not compile `_test.go` files and exits 0 with the unused import still
present (measured, Go 1.24.13). `go vet` and `go test` both report it.
...
they delegate.
  ...
  <acceptance_criteria>
    ...
    - `git diff --name-only 868d45bf873c9e147f18a099171b99afa348f4a3 -- | grep -v '^internal/' | grep -v '^\.planning/'` prints nothing.
  </acceptance_criteria>
...
10. `git diff --name-only 868d45b -- ` lists nothing outside `internal/` and `.planning/` — the Makefile and the CI workflow are untouched.
</verification>
