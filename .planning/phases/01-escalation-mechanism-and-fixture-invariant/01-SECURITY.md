...
| T-01-11 | Elevation of Privilege | Local compose PostgreSQL instance | low | accept | Accepted rationale said "loopback port". **Measured false** — the service binds `0.0.0.0`. Re-accepted with a corrected rationale — see AR-02; **scheduled for fix as SEC-01 (Phase 2)**. | open — below `high` threshold (non-blocking), fix scheduled |
...
- [x] **Operator sign-off on AR-01, AR-02, AR-03** — signed by Alex Zamurko 2026-09-01,
      after requesting all three be quoted in full against their frozen originals. That
      review is what surfaced the AR-01 omission corrected at `359a7fb`. All three replace
      a rationale that measurement contradicted, so this was a decision, not a formality.
- [x] **AR-02 downgraded from acceptance to scheduled fix** — registered as SEC-01
      (Phase 2) at Alex's direction: an accepted risk with a one-character-class fix
      should be a scheduled fix instead.

**Approval:** **verified and signed 2026-09-01 by Alex Zamurko.** Automated audit by
