...
must_haves:
  truths:
    - "With fixtureHasPreamble declared true, TestFixtureIntegrity fails, naming testdata/demo.md, if the pinned fixture ever stops beginning with pre-heading content"
    - "With fixtureHasPreamble declared true, TestFixtureIntegrity fails, naming testdata/demo.md, if that pre-heading content is ever whitespace only"
    - "With fixtureHasPreamble declared true, TestFixtureIntegrity fails, naming testdata/demo.md, if the pinned fixture contains no headings at all — asserted without depending on TestDetectHeadings_Fixture having run first, since Go guarantees no ordering between tests"
    - "fixtureHasPreamble is a pinned constant declared immediately beside fixtureSHA256 and fixtureBytes, so repinning to a preamble-free fixture changes all three in one diff and the declaration can never be made silently"
    - "With fixtureHasPreamble declared false, the two preamble checks do not apply and TestFixtureIntegrity passes against a preamble-free fixture — the declaration is a recorded value, never a skip, and AC-14 content-conditional skips stay green and silent for such a fixture"
    - "The no-headings check survives the declaration: with fixtureHasPreamble false and no headings at all, TestFixtureIntegrity still fails naming testdata/demo.md, because AC-14 indexes headings[0] and a panic is neither green nor silent"
    - "preambleInvariant returns the empty string for an otherwise-violating input when the declaration is false, proven by a dedicated control case, so the gate is exercised in both directions rather than assumed"
  ...
  key_links:
    ...
    - "fixtureHasPreamble is the seam that resolves the policy question: the invariant binds to a declared property of the pinned bytes rather than to an assumption about all fixtures, so a preamble-free fixture is allowed but must be declared, and the declaration is a pin rather than a runtime decision"
  ...
  prohibitions:
    ...
    - "The fixture's pinned SHA-256, byte-length and preamble-declaration constants must not be edited, and testdata/demo.md must not be regenerated, to make the new assertion pass. If the assertion fails, the fixture is wrong, not the assertion."
    - "fixtureHasPreamble must not be flipped to false to silence a failure against the fixture currently pinned. Flipping it is legitimate only as part of a deliberate repin that changes fixtureSHA256 and fixtureBytes in the same diff — it declares a property of the new fixture, it does not waive the assertion for the old one."
...
### The preamble-free fixture is allowed, but must be declared

Alex reviewed the frozen plan set and found the residual: a repin to a genuinely
preamble-free fixture would hard-fail here, where GATE-04 says such a fixture is
legitimate signal. His disposition, recorded 2026-09-01, is to **allow it and
require it to be declared**.

The declaration is a third pinned constant, `fixtureHasPreamble`, declared
immediately beside `fixtureSHA256` and `fixtureBytes`. Repinning the fixture
therefore changes all three constants in one diff, and no reviewer can miss the
claim that a fixture has no preamble.

Declared `true` — the state this phase ships — the invariant applies exactly as
described above. Declared `false`, the preamble assertion does not apply, and
AC-14 declines for that fixture and stays green and silent, which is GATE-04
working as written. **The declaration is a value, never a skip:** the test does
not call `t.Skip`, it evaluates a pinned constant, so a preamble-free fixture is
a recorded decision in the diff rather than a silently absent assertion.

The absent case is closed by the language rather than by a check: the constant is
referenced by `TestFixtureIntegrity`, so deleting it does not weaken the assertion,
it fails to compile. Absent-or-true therefore both resolve to the hard failure, and
the only way to reach the permissive branch is to write `false` down.
</objective>
...
| Const | `fixtureHasPreamble` | **new** — created by this plan in `fixture_test.go`, declared immediately after the two pins below |
...
Measured in this repository at commit `030521b` before planning. Every
measurement holds unchanged at the approved phase base `868d45b`: the two commits
differ only under `.planning/` and present an identical source tree.

- **Phase base commit: `868d45bf873c9e147f18a099171b99afa348f4a3`,** approved by
  Alex Zamurko on 2026-09-01. The cycle-1 base `fcebad1` was mis-anchored and the
  cycle-2 substitution to `030521b` was recorded but never approved; see
  `01-01-PLAN.md` and `01-03-PLAN.md` for both measurements. Every `git diff` in
  this plan is anchored to `868d45b`.
...
<task type="auto">
  ...
  <action>
Add a third pinned constant declared immediately after `fixtureBytes`, alongside
the two existing pins at the top of the file:
`const fixtureHasPreamble = true`. It declares a property of the bytes those two
constants pin, so a repin that changes the fixture changes all three in one diff.
Give it a doc comment saying it is a declaration about the pinned fixture, that
`false` is legitimate for a fixture that genuinely has no preamble, and that
flipping it without also changing the hash and length is silencing an assertion
rather than declaring a fixture.

Add an unexported function
`preambleInvariant(src []byte, headings []Heading, declaredHasPreamble bool) string`
to `fixture_test.go`. It returns the empty string when the invariant holds, and a
one-line failure message otherwise. The gate is a parameter rather than a read of
the package constant so the control test can drive both branches directly.

**The gate waives the two preamble checks, not the no-headings check.** When
`declaredHasPreamble` is false, the first-heading-at-byte-0 and whitespace-only
cases do not apply — that is the declaration doing its job. The no-headings case
still fails. A fixture with no headings at all is not a preamble-free fixture; it
is one where AC-14 indexes `headings[0]` and panics, and a panic is neither green
nor silent. Waiving that check would convert the outcome GATE-04 protects — a
quiet decline — into a crash. Order the function accordingly: no-headings first,
unconditionally, then the gate, then the two preamble cases.

Factoring the decision out of the test body is
...
each on one line and each naming the fixture file. The first applies whatever the
declaration says; the second and third apply only when `declaredHasPreamble` is
true:
...
`detectHeadings` on the bytes `loadFixture` returned, pass those, the headings and
`fixtureHasPreamble` to `preambleInvariant`, and `t.Fatal` the returned message
when it is non-empty. Pass the constant — do not inline `true` at the call site,
which would strand the declaration as an unread value and let a repin flip it with
no effect. Do not guard the call with an `if` on the constant either: the gate
belongs inside the function where the control test can reach it, and a `t.Skip` at
this call site is forbidden outright.

The integrity test must call the function, not reimplement the checks inline — if it
...
mutation-based negative proof cycle 1 proposed. Six cases:
...
- **Declared preamble-free.** The first-heading-at-byte-0 input again, this time
  with `declaredHasPreamble` false. Expect exactly the empty string. This is the
  case that proves the gate is real: without it, a `preambleInvariant` that ignored
  the declaration entirely would pass every other case in this table.
- **Declared preamble-free, but no headings.** An empty heading slice with
  `declaredHasPreamble` false. Expect a non-empty message naming the fixture. The
  declaration admits a fixture with no preamble, never one with no headings — this
  case is what keeps the AC-14 panic guarded on the permissive branch.
...
the message is non-empty and mentions the file, and the two holding cases must
assert the message is exactly empty — the fourth proves the function is not simply
returning a message unconditionally, the fifth proves the declaration gate is wired
rather than decorative, and the sixth proves the gate is narrow rather than a
blanket waiver. The first four cases pass `true`, the last two `false`.
...
this file are where to look. Say that `fixtureHasPreamble` is how a genuinely
preamble-free fixture is admitted — declared beside the hash and length so it
cannot be repinned quietly — and that it is a declared value and not a skip, so
the permissive case is visible in the diff rather than absent from the run.

Change nothing else in the file. Do not touch `loadFixture`, do not edit the two
existing pinning constants — `fixtureHasPreamble` is added beside them, not in
place of them — and do not open the acceptance test for editing.
  </action>
  ...
  <acceptance_criteria>
    ...
    - **The declaration exists and is pinned beside the others.** `grep -n 'const fixtureHasPreamble = true' internal/core/domain/segment/fixture_test.go` prints exactly one line, and its line number is greater than that of `const fixtureBytes` and less than that line plus `12` — the declaration sits with the pins it qualifies, not elsewhere in the file.
    - **The constant is read, not inlined.** `grep -c 'fixtureHasPreamble' internal/core/domain/segment/fixture_test.go` prints at least `3`: the declaration, the argument passed from `TestFixtureIntegrity`, and at least one reference in the control test. `grep -c 'preambleInvariant(.*fixtureHasPreamble' internal/core/domain/segment/fixture_test.go` prints at least `1` — the integrity test passes the constant on the call line, never an inlined literal. (The control test does pass literals, by design, so do not assert the absence of `true` file-wide.)
    - **The declaration is not a skip.** `grep -c 't\.Skip' internal/core/domain/segment/fixture_test.go` prints `0`, the same as at the phase base. The permissive branch is a returned empty string from a pure function, never a skipped test.
    - **The gate is proven in both directions.** The fifth control subtest passes the zero-offset violating input with `declaredHasPreamble` false and asserts the return is exactly empty; it reports PASS. A `preambleInvariant` that ignored its third parameter would fail it while passing all four others.
    - **The gate is narrow, not a blanket waiver.** The sixth control subtest passes an empty heading slice with `declaredHasPreamble` false and asserts the return is non-empty and names the fixture; it reports PASS. A gate that returned early on `false` before the no-headings check would fail it.
    - `grep -v '^\s*//' internal/core/domain/segment/fixture_test.go | grep -c 'strings.TrimSpace'` prints at least `1`.
    - `grep -c 'detectHeadings' internal/core/domain/segment/fixture_test.go` is strictly greater than `git show 868d45bf873c9e147f18a099171b99afa348f4a3:internal/core/domain/segment/fixture_test.go | grep -c detectHeadings`.
    - **The negative proof is mutation-free.** `git diff --name-only 868d45bf873c9e147f18a099171b99afa348f4a3 --` prints exactly `internal/core/domain/segment/fixture_test.go` and nothing else, at every point during and after this task. No command in this task moves, renames, or temporarily rewrites any file.
...
calling one pure function, gated on the `fixtureHasPreamble` declaration, and that
same function is exercised against six inputs: three that violate the invariant,
one that satisfies it, one that violates it but is declared preamble-free and so
passes, and one that is declared preamble-free yet still fails for having no
headings. The assertion is therefore proven able to both fail and pass, and the
declaration gate proven both to open and to stay narrow — without any file in the
working tree having been mutated and restored. The segment package still passes
...
stated baseline rather than an assumption.
  ...
  <verify>
    <automated>cd C:/EpistemicOS/epistemicos-gsd-pilot &amp;&amp; set -o pipefail &amp;&amp; git diff --quiet 868d45bf873c9e147f18a099171b99afa348f4a3 -- internal/core/domain/segment/acceptance_test.go &amp;&amp; go test ./internal/core/domain/segment/... -count=1 -run TestAC14_PreHeadingContentHasNoNode -v 2>&amp;1 | grep -q '^--- PASS: TestAC14_PreHeadingContentHasNoNode' &amp;&amp; echo AC14_INTACT</automated>
  </verify>

  <acceptance_criteria>
    - `git diff --name-only 868d45bf873c9e147f18a099171b99afa348f4a3 -- internal/core/domain/segment/acceptance_test.go` prints nothing — the file is byte-identical to the phase base.
    - `grep -c 't.Skip' internal/core/domain/segment/acceptance_test.go` prints the same number as `git show 868d45bf873c9e147f18a099171b99afa348f4a3:internal/core/domain/segment/acceptance_test.go | grep -c 't.Skip'`.
    ...
    - `git diff --name-only 868d45bf873c9e147f18a099171b99afa348f4a3 --` lists exactly `internal/core/domain/segment/fixture_test.go` for this plan's work, and no other path.
  </acceptance_criteria>
...
5. `git diff --name-only 868d45b -- internal/core/domain/segment/acceptance_test.go` prints nothing.
...
8. `git diff --name-only 868d45b --` for this plan lists exactly one file.
</verification>
...
to the phase base commit `868d45b`; and the deferred AC-14 empty-heading guard, so
