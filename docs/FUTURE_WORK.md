...
| W | **Four section types have no role in the table at all.** Measured on the wedding-vendor paper (`ea07e5f5`), which leaves 11 of 39 sections unclassified: `Problem statement`, `Purpose statement`, `Research questions` and `Academic value and contribution` match **zero** keywords. These are not mangled headings or ties — they are a standard structure in education and business research that the table does not know about. Unlike the weak-keyword tier (parked item 3), this needs new roles or new keywords on existing ones, not a looser matcher. Check the other thirteen papers before choosing which. | `domain/segment` table | hours |
